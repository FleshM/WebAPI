...
router_contacts = APIRouter(prefix='/items', tags=['item'])


@router_contacts.post("/", response_model=schemas.Contact)
...
@router_contacts.get("/{contact_id}", response_model=schemas.Contact)
async def read_contact(contact_id: int, db: Session = Depends(get_db)):
    contact = crud.get_contact(db, contact_id)
    return contact


@router_contacts.get("/", response_model=List[schemas.Contact])
